...
def test_run_scan_mocks_coral(mocker) -> None:
    ...
    rows = run_scan(ecosystem="PyPI", packages=["django"])
    assert len(rows) == 1
    assert rows[0]["cve"] == "GHSA-x"
...
def test_run_correlate_adds_signal(mocker) -> None:
    ...
    rows = run_correlate(ecosystem="PyPI", packages=["django"], since="7d")
    assert rows[0]["signal"] == "active"
